...
There are two main files to list the versions of a package:

- For ConanCenter build service we have `config.yml`
- Within the recipe we have `conandata.yml`
...
`files.apply_conandata_patches` is the best option.
...
For more complicated cases, `files.rm` or `files.replace_in_file` are allowed.

```py
def _patch_sources(self):
    # remove bundled libfmt
    files.rmdir(self, os.path.join(self.source_folder, "lib", "fmt"))
    files.replace_in_file(self, os.path.join(self.source_folder, "CMakeLists.txt"), "${CMAKE_SOURCE_DIR}", "${CMAKE_CURRENT_SOURCE_DIR}")
```
